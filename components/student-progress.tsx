    ...
    const fetchStudentData = async () => {
      ...
      try {
        ...
        const setoranQuery = query(
          collection(db, "users", currentUser.uid, "setoran"),
  ...
  return (
    <Card>
      <CardHeader className="flex flex-row items-center justify-between">
        <CardTitle>Progres Hafalan Santri</CardTitle>
        <Button onClick={handlePrint} variant="outline" size="sm" className="flex items-center gap-2 bg-transparent">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              <Card>
                ...
                <CardContent className="space-y-2">
                  <p>
                    <strong>Total Ayat Dihafal:</strong> {selectedStudentSummary.totalMemorizedVerses}
                  </p>
                  <p>
                    <strong>Total Ayat Murajaah:</strong> {selectedStudentSummary.totalMurajaahVerses}
                  </p>
                  <p>
                    <strong>Progres Keseluruhan Juz:</strong> {selectedStudentSummary.juzProgress.completedJuz} /{" "}
                    {selectedStudentSummary.juzProgress.totalJuz} Juz
                  </p>
                ...
                <CardContent className="space-y-2 max-h-60 overflow-y-auto">
                  {Object.entries(selectedStudentSummary.juzProgress.details).map(([juz, data]) => (
                    <div key={juz} className="flex items-center gap-2">
                      ...
                      <span>
                        {data.memorizedVerses}/{data.totalVerses} ayat ({data.progress.toFixed(0)}%)
                      </span>
                  ...
                  <TableBody>
                    {selectedStudent?.hafalan && selectedStudent.hafalan.length > 0 ? (
                      selectedStudent.hafalan.map((h, index) => (
                        <TableRow key={index}>
                          <TableCell>
                            {QURAN_DATA.find((s) => s.name.transliteration.id === h.surah)?.name.transliteration.id ||
                              h.surah}
                          </TableCell>
                          <TableCell>
                            {h.start}-{h.end}
                          ...
                          <TableCell>
                            <Badge className={getPenilaianClass(h.penilaian)}>{h.penilaian}</Badge>
                          </TableCell>
                        </TableRow>
                      ))
                    ) : (
